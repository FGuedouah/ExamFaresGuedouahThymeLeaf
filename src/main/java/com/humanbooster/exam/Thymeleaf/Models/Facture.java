...
public class Facture {
    ...
    public BigDecimal getTotalHT(){
        ...
        for(LigneFacture ligne : getLignes()){
            BigDecimal prixHt = ligne.getPrixHt();
            BigDecimal quantity = ligne.getQuantity();
            totalHT = totalHT.add(prixHt.multiply(quantity));
        }
        return totalHT;
    }
    ...
    public BigDecimal getTotalTTC(){
        ...
        for(LigneFacture ligne : getLignes()){
            BigDecimal prixHt = ligne.getPrixHt();
            BigDecimal quantity = ligne.getQuantity();
            totalTTC = totalTTC.add((prixHt.multiply(quantity)).multiply(BigDecimal.valueOf(ligne.getTva().getTaux() + 1)));
        }
        return totalTTC;
    }
}
